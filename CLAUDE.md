...
### 2025-01-09 - GoMC版本实现
- **新版本开发**：
  - 基于go-mc库重新实现服务器，提供更好的兼容性
  - 创建`internal/protocol/gomc_handler.go`处理器
  - 创建`cmd/server-gomc/main.go`独立入口
  - 保留原版代码作为备份
- **技术优势**：
  - 使用go-mc的标准服务器框架和协议处理
  - 完全符合Minecraft协议标准
  - 更可靠的连接处理和错误处理
  - 更简洁的代码，利用go-mc的高级抽象
- **功能完整性**：
  ...
  - 兼容现有配置文件
- **文档**：
  - 创建详细的GoMC版本文档`docs/gomc-version-readme.md`
  - 包含架构设计、使用指南、迁移指南等

### 2025-01-09 - 兼容性修复和连接处理优化（原版）
- **兼容性问题修复**：
  - 放宽数据包大小检查：最小包大小从2字节改为1字节
  - 增加1字节数据包处理：直接发送状态响应，兼容简单查询工具
  - 改进未知协议包处理：不立即拒绝，尝试发送状态响应
- **连接处理优化**：
  - 移除过于严格的连接状态检查：避免误判正常连接为已关闭
  - 简化响应发送逻辑：直接尝试发送，只在真正失败时报错
  - 优化错误处理：减少"connection closed"误报
- **问题解决**：
  - 修复"packet too small: 1"错误，提高状态查询网站兼容性
  - 减少连接关闭导致的响应发送失败
  - 提升对非标准查询工具的支持
  - 解决过度连接检查导致的"connection closed"错误
